...
const pick = (r, ...cols) => {
	for (const c of cols) if (numv(r[c])) return numv(r[c]);
	return 0;
};
...
for (const rn of rowNums) {
	...
	const hoursDay = pick(r, 'K', 'AP');
	const hoursNight = pick(r, 'L', 'AQ');
	...
	const flight = {
		...
		hoursDay,
		hoursNight,
		hoursSil: pick(r, 'M', 'AR'),
		hoursIFR: numv(r.AS),
		hoursCAG: numv(r.N),
		hoursCAM: numv(r.O),
		hoursMEDay: pick(r, 'P', 'AU'),
		hoursMENight: pick(r, 'Q', 'AV'),
		landings: numv(r.J),
